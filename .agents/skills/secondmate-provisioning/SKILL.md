...
`/updatefirstmate` first updates the remote code root from that checkout's own update remote - `upstream` when the fork there carries nothing of its own, else that checkout's `origin` - then runs that guarded home sync.

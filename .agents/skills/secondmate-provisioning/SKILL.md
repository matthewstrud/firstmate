...
`/updatefirstmate` first updates the remote code root from that checkout's own update remote, resolved from that host's remotes rather than inherited from the primary, then runs that guarded home sync.
[`updatefirstmate`](../updatefirstmate/SKILL.md) owns which remote that resolves to.

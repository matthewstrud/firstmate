...
Effort values unsupported by a harness with an effort axis refuse the `fm-spawn.sh` launch, while harnesses without a separate effort axis keep the requested value in task meta and omit the flag.
That keeps unsupported effort requests visible while preserving the deliberate no-axis behavior for opencode, kimi, and cursor.

...
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for update-remote pulls (`upstream` when the fork carries nothing of its own, else `origin`) and local secondmate syncs |

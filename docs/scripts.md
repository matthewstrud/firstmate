...
| `fm-tool-check.sh`       | One-shot presence report for the tools named on its command line, with the resolved path and `--version` line of each one that resolves to a file |

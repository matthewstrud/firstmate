...
A standalone-clone home cannot receive a primary-local commit through that no-fetch sync, so it receives the rule through `/updatefirstmate`'s update-remote refresh instead, resolved from that home's own remotes (`upstream` when the fork carries nothing of its own, else `origin`).

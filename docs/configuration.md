...
If a selected profile carries an effort value for a harness with no separate effort axis, `fm-spawn.sh` records the requested `effort=` in task meta for traceability but omits the launch flag.
If a selected profile carries an unsupported effort value for a harness with an effort axis, `fm-spawn.sh` refuses the launch instead of silently dropping the flag, and bootstrap reports the invalid harness/effort pair as a `CREW_DISPATCH` diagnostic when it is visible in the file.

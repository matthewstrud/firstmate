...
retired conditional value as a task mode|--mode no-mistakes-prod-only --yolo off|retired registry value
ROWS
...
# unregistered project resolves to the direct-PR standing default (AGENTS.md
# section 7), so shipping it direct-PR is quiet while local-only is announced.
# The retired conditional value reads as direct-PR and is never announced.
...
retired conditional value shipped direct-PR|- proj [no-mistakes-prod-only] - fixture (added 2026-01-01)|direct-PR|quiet|no-mistakes-prod-only
unregistered project shipped direct-PR is quiet|- other [no-mistakes] - fixture (added 2026-01-01)|direct-PR|quiet|direct-PR
...
unbracketed legacy entry reads as direct-PR|- proj - fixture (added 2026-01-01)|direct-PR|quiet|direct-PR
ROWS
...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  status=$?
  [ "$status" -ne 0 ] || fail "promotion on the retired conditional value should exit non-zero"
  assert_contains "$out" "retired registry value" "promote did not refuse the retired conditional value as a task mode"
...
# The registry parser survives for the mechanical consumers only. It still accepts
# the retired conditional value, maps it to the direct-PR it was converted to, and
# exposes the raw annotation. No default or fallback ever yields no-mistakes: only
# an explicit (captain-requested) no-mistakes entry does.
...
- nmproj [no-mistakes] - fixture (added 2026-01-01)
EOF
  out=$(FM_HOME="$home" "$PROJECT_MODE" prodproj 2>/dev/null)
  [ "$out" = "direct-PR off" ] || fail "retired conditional value did not map to direct-PR (got '$out')"
  ...
  [ "$out" = "direct-PR on" ] || fail "retired conditional value dropped its +yolo posture (got '$out')"
  ...
  [ "$out" = "direct-PR off" ] || fail "a typo'd mode did not fall back to the direct-PR default (got '$out')"
  ...
  [ "$out" = "direct-PR off" ] || fail "an unbracketed legacy entry did not read as direct-PR (got '$out')"
  ...
  out=$(FM_HOME="$home/absent" "$PROJECT_MODE" prodproj 2>/dev/null)
  [ "$out" = "direct-PR off" ] || fail "an absent registry did not default to direct-PR (got '$out')"
  out=$(FM_HOME="$home" "$PROJECT_MODE" nmproj 2>/dev/null)
  [ "$out" = "no-mistakes off" ] || fail "an explicit no-mistakes entry no longer reads as no-mistakes (got '$out')"
  pass "fm-project-mode: only an explicit no-mistakes entry yields no-mistakes; defaults and the retired value read as direct-PR"
}

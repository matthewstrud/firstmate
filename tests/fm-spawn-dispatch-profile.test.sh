...
test_codex_threads_max_effort() {
  ...
  assert_contains "$launch" "codex --model 'gpt-5' -c 'model_reasoning_effort=\"max\"' --dangerously-bypass-approvals-and-sandbox" \
    "codex launch did not apply max reasoning effort"
  pass "codex receives the max model_reasoning_effort override"
}
...
test_grok_rejects_invalid_max_reasoning_effort() {
  local rec id out status launch
  ...
  status=$?
  expect_code 1 "$status" "grok spawn with unsupported max reasoning effort should fail loudly"
  assert_contains "$out" "harness 'grok' does not support effort 'max'" \
    "grok max refusal did not name the unsupported effort"
  assert_absent "$HOME_DIR/state/$id.meta" "grok max refusal should happen before metadata publication"
  launch=$(cat "$LAUNCH_LOG")
  [ -z "$launch" ] || fail "grok max refusal must not compose or send a launch"
  pass "grok rejects unsupported max reasoning effort loudly"
}

test_grok_rejects_invalid_xhigh_reasoning_effort() {
  local rec id out status launch
  ...
  status=$?
  expect_code 1 "$status" "grok spawn with unsupported xhigh reasoning effort should fail loudly"
  assert_contains "$out" "harness 'grok' does not support effort 'xhigh'" \
    "grok xhigh refusal did not name the unsupported effort"
  assert_absent "$HOME_DIR/state/$id.meta" "grok xhigh refusal should happen before metadata publication"
  launch=$(cat "$LAUNCH_LOG")
  [ -z "$launch" ] || fail "grok xhigh refusal must not compose or send a launch"
  pass "grok rejects unsupported xhigh reasoning effort loudly"
}

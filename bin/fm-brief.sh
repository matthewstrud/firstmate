...
Optional helper: \`bin/fm-secondmate-report.sh [--key <key>] <verb> <corr_id> <note>\` appends that correlated line to the parent channel itself - do not pass a status path, and do not write a hand path under this home. A \`--key <key>\` option places \`[key=<key>]\` at the note head (right after the colon), the position bin/fm-classify-lib.sh reads when the leading bracket carries no key, because the corr token already occupies that bracket; the decision then opens and closes under that key in the parent's OPEN DECISIONS fold, so the main firstmate can clear it with \`fm-send.sh --resolve-key <key>\`. The helper rejects \`default\` and any key in the reserved \`pending-reply-\` namespace, so a key it accepts is always one the fold can open and \`fm-send.sh --resolve-key\` can close. Do not place a key at the tail of the note: the fold ignores it there and the decision silently falls under the default key.

...
Optional helper: \`bin/fm-secondmate-report.sh [--key <key>] <verb> <corr_id> <note>\` appends that correlated line to the parent channel itself - do not pass a status path, and do not write a hand path under this home. A \`--key <key>\` option places \`[key=<key>]\` at the note head (right after the colon), the only bracket position that opens or closes a keyed decision in the parent's OPEN DECISIONS fold (bin/fm-classify-lib.sh), so the main firstmate can clear it with \`fm-send.sh --resolve-key <key>\` instead of the default key; the corr token stays in the leading bracket so the reply still correlates.
